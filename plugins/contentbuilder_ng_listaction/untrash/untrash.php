...
class plgContentbuilder_ng_listactionUntrash extends CMSPlugin implements SubscriberInterface
{
    ...
    function onBeforeAction($form_id, $record_ids)
    {
        ...
        $lang->load('plg_contentbuilder_ng_listaction_untrash', JPATH_ADMINISTRATOR);

        foreach ($record_ids as $record_id) {

            $db->setQuery("Update #__content As content, #__contentbuilder_ng_records As record, #__contentbuilder_ng_articles As article Set content.state = record.published Where article.record_id = record.record_id And article.form_id = " . intval($form_id) . " And article.record_id = " . $db->Quote($record_id) . " And content.id = article.article_id");
            $db->execute();
        }

        Factory::getApplication()->enqueueMessage(Text::_('COM_CONTENTBUILDER_NG_UNTRASH_SUCCESSFULL'));
