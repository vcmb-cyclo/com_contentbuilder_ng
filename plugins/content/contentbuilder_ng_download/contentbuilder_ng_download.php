...
class plgContentContentbuilder_ng_download extends CMSPlugin implements SubscriberInterface
{
    /**
     * Application object.
     *
	...
	function onContentPrepare($context = '', $article = null, $params = null, $limitstart = 0, $is_list = false, $form = null, $item = null)
    {
        ...
        $plugin = PluginHelper::getPlugin('content', 'contentbuilder_ng_download');
    //    $params = new Registry;
    //    $pluginParams = $params->loadString($plugin->params);
        ...
        $lang->load('plg_content_contentbuilder_ng_download', JPATH_ADMINISTRATOR);
        ...
        if (!is_dir(JPATH_SITE .'/media/contentbuilder_ng/plugins/download')) {
            Folder::create(JPATH_SITE .'/media/contentbuilder_ng/plugins/download');
        }

        if (!file_exists(JPATH_SITE .'/media/contentbuilder_ng/plugins/download/index.html'))
                ...
                if (!$is_list) {
                    ...
                    if ($frontend) {
                        if (!ContentbuilderLegacyHelper::authorizeFe('view')) {
                            if (Factory::getApplication()->input->get('contentbuilder_ng_download_file', '', 'GET', 'STRING', CBREQUEST_ALLOWRAW, 'string')) {
                                ob_end_clean();
                                die('No Access');
                            } else {
                                return true;
                                ...
                                for ($fcnt = 0; $fcnt < $the_files_size; $fcnt++) {
                                    ...
                                    if ($the_value) {
                                        ...
                                        if ($exists) {
                                            ...
                                            if (Factory::getApplication()->input->get('contentbuilder_ng_download_file', '', 'GET', 'STRING', CBREQUEST_ALLOWRAW, 'string') == sha1($field . $the_value)) {

                                                $download_name = basename(OutputFilter::stringURLSafe($default_title) . '_' . $the_value);
                                                $file_id = md5($type . $item->recElementId . $the_value);

                                                if (!$this->app->getSession()->get('downloaded' . $type . $item->recElementId . $file_id, false, 'com_contentbuilder_ng.plugin.download')) {

                                                    $this->db->setQuery("Select hits From #__contentbuilder_ng_resource_access Where `type` = " . $this->db->Quote($type) . " And resource_id = '" . $file_id . "' And element_id = " . $this->db->Quote($item->recElementId));
                                                    if ($this->db->loadResult() === null) {
                                                        $this->db->setQuery("Insert Into #__contentbuilder_ng_resource_access (`type`, form_id, element_id, resource_id, hits) values (" . $this->db->Quote($type) . "," . intval($form_id) . ", " . $this->db->Quote($item->recElementId) . ", '" . $file_id . "',1)");
                                                    } else {
                                                        $this->db->setQuery("Update #__contentbuilder_ng_resource_access Set `type` = " . $this->db->Quote($type) . ", resource_id = '" . $file_id . "', form_id = " . intval($form_id) . ", element_id = " . $this->db->Quote($item->recElementId) . ", hits = hits + 1 Where `type` = " . $this->db->Quote($type) . " And resource_id = '" . $file_id . "' And element_id = " . $this->db->Quote($item->recElementId));
                                                    }
                                                    $this->db->execute();
                                                }

                                                $this->app->getSession()->set('downloaded' . $type . $item->recElementId . $file_id, true, 'com_contentbuilder_ng.plugin.download');
                                            ...
                                            $this->db->setQuery("Select hits From #__contentbuilder_ng_resource_access Where resource_id = '" . $file_id . "' And `type` = " . intval($type) . " And element_id = " . $this->db->Quote($item->recElementId));
                                            ...
                                            $open_ = Route::_($url . (strstr($url, '?') !== false ? '&' : '?') . 'contentbuilder_ng_download_file=' . sha1($field . $the_value));

                                            $out .= '<div style="' . ($align_ ? 'float: ' . $align_ . ';' : '') . str_replace('::', ';', $box_style_) . '">
                                                        <a href="' . $open_ . '">' . Text::_('COM_CONTENTBUILDER_NG_PLUGIN_DOWNLOAD_DOWNLOAD') . '</a>'
                                                . ($info_ ?
                                                    '<div style="' . (str_replace('::', ';', $info_style_)) . '">
                                                                    ' . ($hide_filename_ ? '' : '<span class="cbPluginDownloadFilename">' . Text::_('COM_CONTENTBUILDER_NG_PLUGIN_DOWNLOAD_FILENAME') . ':</span> ' . $download_name . '<br/>') . '
                                                                    ' . ($hide_mime_ ? '' : '<span class="cbPluginDownloadMime">' . Text::_('COM_CONTENTBUILDER_NG_PLUGIN_DOWNLOAD_MIME') . ':</span> ' . $mime . '<br/>') . '
                                                                    ' . ($hide_size_ ? '' : '<span ' . ($hide_size_ ? ' style="display:none;" ' : '') . 'class="cbPluginDownloadSize">' . Text::_('COM_CONTENTBUILDER_NG_PLUGIN_DOWNLOAD_SIZE') . ':</span> ' . $size . '<br/>') . '
                                                                    ' . ($hide_downloads_ ? '' : '<span ' . ($hide_downloads_ ? ' style="display:none;" ' : '') . 'class="cbPluginDownloadDownloads">' . Text::_('COM_CONTENTBUILDER_NG_PLUGIN_DOWNLOAD_DOWNLOADS') . ':</span> ' . $hits . '<br/>') . '
                                                                 </div>' : '') . '</div>';
