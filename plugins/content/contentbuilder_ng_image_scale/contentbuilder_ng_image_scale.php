...
class plgContentContentbuilder_ng_image_scale extends CMSPlugin implements SubscriberInterface
{
	...
	function onContentPrepare($context = '', $article = null, $params = null, $limitstart = 0, $is_list = false, $form = null, $item = null)
	{
		...
		$plugin = PluginHelper::getPlugin('content', 'contentbuilder_ng_image_scale');
		...
		if (!is_dir(JPATH_SITE .'/media/contentbuilder_ng/plugins')) {
			Folder::create(JPATH_SITE .'/media/contentbuilder_ng/plugins');
		}

		if (!file_exists(JPATH_SITE .'/media/contentbuilder_ng/plugins/index.html'))
			File::write(JPATH_SITE .'/media/contentbuilder_ng/plugins/index.html', $def = '');

		if (!is_dir(JPATH_SITE .'/media/contentbuilder_ng/plugins/image_scale')) {
			Folder::create(JPATH_SITE .'/media/contentbuilder_ng/plugins/image_scale');
		}
		...
		if (isset ($article->id) || isset ($article->cbrecord)) {
			...
			if (isset ($matches[0]) && is_array($matches[0]) && isset ($matches[1]) && is_array($matches[1])) {
				...
				if (isset ($article->id) && $article->id && !isset ($article->cbrecord)) {
					...
					$this->db->setQuery("Select form.`title_field`,form.`protect_upload_directory`,form.`reference_id`,article.`record_id`,article.`form_id`,form.`type`,form.`published_only`,form.`own_only`,form.`own_only_fe` From #__contentbuilder_ng_articles As article, #__contentbuilder_ng_forms As form Where form.`published` = 1 And form.id = article.`form_id` And article.`article_id` = " . $this->db->quote($article->id));
				...
				if (!file_exists(JPATH_SITE .'/media/contentbuilder_ng/plugins/image_scale/index.html'))
					File::write(JPATH_SITE .'/media/contentbuilder_ng/plugins/image_scale/index.html', $def = '');

				if (!is_dir(JPATH_SITE .'/media/contentbuilder_ng/plugins/image_scale/cache')) {
					Folder::create(JPATH_SITE .'/media/contentbuilder_ng/plugins/image_scale/cache');
				}

				if (!file_exists(JPATH_SITE .'/media/contentbuilder_ng/plugins/image_scale/cache/index.html'))
					File::write(JPATH_SITE .'/media/contentbuilder_ng/plugins/image_scale/cache/index.html', $def = '');

				if (!is_dir(JPATH_SITE .'/media/contentbuilder_ng/plugins/image_scale/cache/' . $form_id)) {
					Folder::create(JPATH_SITE .'/media/contentbuilder_ng/plugins/image_scale/cache/' . $form_id);
				}

				if (!file_exists(JPATH_SITE .'/media/contentbuilder_ng/plugins/image_scale/cache/' . $form_id .'/index.html')){
					File::write(JPATH_SITE .'/media/contentbuilder_ng/plugins/image_scale/cache/' . $form_id .'/index.html', $def = '');
				}

				if ($protect) {
					if (!file_exists(JPATH_SITE .'/media/contentbuilder_ng/plugins/image_scale/cache/' . $form_id .'/.htaccess'))
						File::write(JPATH_SITE .'/media/contentbuilder_ng/plugins/image_scale/cache/' . $form_id .'/.htaccess', $def = 'deny from all');
				} else {
					if (file_exists(JPATH_SITE .'/media/contentbuilder_ng/plugins/image_scale/cache/' . $form_id .'/.htaccess')) {
						File::delete(JPATH_SITE .'/media/contentbuilder_ng/plugins/image_scale/cache/' . $form_id .'/.htaccess');
					}
				}

				$default_folder = JPATH_SITE .'/media/contentbuilder_ng/plugins/image_scale/cache/' . $form_id;
				...
				if (!$is_list) {
					...
					if ($frontend) {
						if (!ContentbuilderLegacyHelper::authorizeFe('view')) {
							if (Factory::getApplication()->input->getInt('contentbuilder_ng_display', 0) || ($protect && Factory::getApplication()->input->getInt('contentbuilder_ng_display_detail', 0))) {
								ob_end_clean();
								die ('No Access');
							} else {
								return true;
				...
				foreach ($matches[1] as $match) {
										...
										for ($fcnt = 0; $fcnt < $the_files_size; $fcnt++) {
											...
											if ($the_value && ($width || $height)) {
												...
												if ($image !== false) {
													...
													if (Factory::getApplication()->input->getInt('contentbuilder_ng_display_detail', 0)) {

														if (Factory::getApplication()->input->get('contentbuilder_ng_detail_file', '', 'REQUEST', 'STRING', CBREQUEST_ALLOWRAW, 'string') == sha1($field . $the_value)) {
													...
													if ($filename) {
														...
														if ($the_image !== false) {
															if (Factory::getApplication()->input->getInt('contentbuilder_ng_display', 0)) {

																if (Factory::getApplication()->input->get('contentbuilder_ng_field', '', 'REQUEST', 'STRING', CBREQUEST_ALLOWRAW, 'string') == sha1($field . $filename)) {
																...
																if (trim($open) == 'true') {
																	if ($protect) {
																		$open_ = Route::_($url . (strstr($url, '?') !== false ? '&' : '?') . 'contentbuilder_ng_display_detail=1&contentbuilder_ng_detail_file=' . sha1($field . $the_value));
																	} else {
																...
																if ($protect) {
																	$src = Route::_($url . (strstr($url, '?') !== false ? '&' : '?') . 'contentbuilder_ng_display=1&contentbuilder_ng_field=' . sha1($field . $filename));
																} else {
					...
					if (trim($out) == '' && file_exists(JPATH_SITE .'/media/contentbuilder_ng/plugins/image_scale/' . basename($default_image))) {
						$out = '<img width="' . $default_image_width . '" height="' . $default_image_height . '" alt="" src="' . Uri::root(true) . '/media/contentbuilder_ng/plugins/image_scale/' . basename($default_image) . '"/>';
					}
